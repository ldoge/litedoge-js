...
class Wallet extends EventEmitter {
  ...
  async createCoinStake(bits, time, searchInterval) {
    ...
    for (const unReservedCredit of unReservedCredits) {
      ...
      const kernel = this.findKernel(prev, bits, blockFrom, prevTx, coin, time, interval)
      if (!kernel) break;
    ...
    const fundedAmount = this.fundCoinstake(coinStakeTx, unReservedCredits, credit, walletKey, balance);
    credit += fundedAmount;
    const coinAge = coinStakeTx.getCoinAge(this.txdb);
    if (!coinAge)
    ...
    //Sign
    const signatureCount = coinStakeTx.sign(walletKey);
