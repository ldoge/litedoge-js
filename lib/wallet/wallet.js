...
const {Input, Output} = require('../primitives');
...
const StakeSplitAge = 9 * 24 * 60 * 60;
const StakeCombineThreshold = 50n * consensus.COIN;
...
class Wallet extends EventEmitter {
   ...
   * @returns {module:primitives.TX|module:primitives.TX}
   */
  async createCoinStake(bits, time, searchInterval) {
    const logger = this.logger.context('coinstake');
    let prevHeight = this.wdb.state.height;

    // const targetPerCoinDay = consensus.fromCompact(bits);
    const coinStakeTx = new TX({time: time});
    ...
    for (const unReservedCredit of unReservedCredits) {
      ...
      const chainEntry = await this.wdb.getEntry(coin.height);
      const prevTx = await this.txdb.getTX(coin.hash);

      if (chainEntry.time + this.network.pos.stakeMinAge > coinStakeTx.time - maxStakeSearchInterval)
        continue; // only count coins meeting min age requirement

      let kernelFound = false;
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prevHeight === this.wdb.state.height; n++) {

        // Search backward in time from the given txNew timestamp
        // Search nSearchInterval seconds back up to nMaxStakeSearchInterval
        const kernel = this.kernel.checkStakeKernelHash(prev, job.bits, chainEntry.time, prevTx.tx, unReservedCredit.coin, coinStakeTx - n)
        if (kernel != null) {
          logger.info("Woah! We found a kernel!");
          const scriptPubKeyOut = this.getCoinPubKey(coin);
          ...
          if (this.kernel.getStakeWeight(chainEntry.time, coinStakeTx.time) < StakeSplitAge)
            coinStakeTx.addOutput(Output.fromScript(scriptPubKeyOut, 0n)); //split stake
          logger.info("CreateCoinStake : added kernel type=%d", whichType);
          kernelFound = true;
          break;
   ...
   */
  getCoinPubKey(coin) {
    const scriptPubKeyKernel = coin.script;

    const scriptSolution = scriptPubKeyKernel.solve();
    const {whichType, solutions} = scriptSolution;

    if (!scriptSolution) {
      logger.error("CreateCoinStake : failed to parse kernel");
      return null;
    }

    logger.info("CreateCoinStake : parsed kernel type=%d", whichType);
    if (whichType !== Script.types.PUBKEY && whichType !== Script.types.PUBKEYHASH) {
      logger.error("CreateCoinStake : no support for kernel type=%d", whichType);
      return null;  // only support pay to public key and pay to address
    }

    // pay to address type
    if (whichType === Script.types.PUBKEYHASH) {
      // convert to pay to public key type
      //TODO:: figure out how to do this
      const key = this.getKey(solutions[0]);
      if (!key) {
        logger.error("CreateCoinStake : failed to get key for kernel type=%d", whichType);
        return null  // unable to find corresponding public key
      }
      return Script.fromPubkey(key.publicKey);
    }
    //
    if (whichType === Script.types.PUBKEY) {
      const vchPubKey = solutions[0];
      const key = this.getKey(vchPubKey);
      if (!key) {
        logger.error("CreateCoinStake : failed to get key for kernel type=%d", whichType);
        return null;  // unable to find corresponding public key
      }
      ...
      // keys mismatch
      if (key.publicKey !== vchPubKey) {
        logger.error("coinstake", "CreateCoinStake : invalid key for kernel type=%d\n", whichType);
        return null
      }
