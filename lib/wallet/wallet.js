...
const {Input, Output, Address, TX} = require('../primitives');
...
class Wallet extends EventEmitter {
   ...
   *
   * @param acct
   * @returns {Promise<module:primitives.Credit[]>}
   */
  ...
  async createCoinStake(bits, time, searchInterval) {
    ...
    for (const unReservedCredit of unReservedCredits) {
      ...
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prevHeight === this.wdb.state.height; n++) {
        ...
        const kernel = this.kernel.checkStakeKernelHash(prev, bits, chainEntry.time, prevTx.tx, unReservedCredit.coin, coinStakeTx - n)
        if (kernel != null) {
