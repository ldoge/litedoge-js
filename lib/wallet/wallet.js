  ...
  async createCoinStake(bits, time, searchInterval) {
    ...
    for (const credit of unReserved) {
      ...
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prevHeight === this.wdb.state.height; n++) {
        ...
        const kernel = this.kernel.checkStakeKernelHash(prev, job.bits, chainEntry.time, prevTx.tx, credit.coin, coinStakeTx - n)
        if (kernel != null) {
          this.logger.info("Woah! We found a kernel!");
           // this.sign()
