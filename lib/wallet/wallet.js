...
const {Input, Output, Address, TX, MTX} = require('../primitives');
...
class Wallet extends EventEmitter {
  ...
  async createCoinStake(bits, time, searchInterval) {
    ...
    for (const unReservedCredit of unReservedCredits) {
      ...
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prev.height === this.wdb.state.height; n++) {

        // Search backward in time from the given txNew timestamp
        // Search nSearchInterval seconds back up to nMaxStakeSearchInterval
        const kernel = this.kernel.checkStakeKernelHash(prev, bits, blockFrom, prevTx.tx, unReservedCredit.coin, coinStakeTx.time - n)
        if (true || kernel != null) {
          logger.info("Woah! We found a kernel!");
          const scriptPubKeyOut = await this.getCoinSigningKey(coin);
