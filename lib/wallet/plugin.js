...
const StakingKernel = require('../staking/stakingkernel')
/**
...
class Plugin extends EventEmitter {
  ...
  constructor(node) {
    ...
    this.stakingKernel = new StakingKernel({
      version: 7,
  ...
  init() {
    ...
    this.stakingKernel.on('error', err => this.emit('error', err))
  }

  async open() {
    await this.wdb.open();
    this.rpc.wallet = this.wdb.primary;
    this.stakingKernel.wallet = this.wdb.primary;
    await this.stakingKernel.open();
  ...
  async close() {
    ...
    this.stakingKernel.wallet = null;
    await this.stakingKernel.close();
