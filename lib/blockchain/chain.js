...
class Chain extends AsyncEmitter {
   ...
   * Get previous entry.
   * @param {ChainEntry} entry
   * @param {Boolean} proofOfStake
   * @returns {Promise} - Returns ChainEntry.
   */

  getPrevious(entry, proofOfStake = false) {

    return this.db.getPrevious(entry, proofOfStake);
  }
  ...
  async verify(block, prev, flags) {
    ...
    const prevForTarget = await this.getPrevious(ChainEntry.fromBlock(block), block.isProofOfStake());
    const bits = await this.getTarget( block.time, prevForTarget, block.isProofOfStake());
  ...
  async getTarget(time, prev, isProofOfStake) {
    const pow = this.network.pow;
    ...
    const prev1 = this.getPrevCache(prev, isProofOfStake) || await this.getPrevious(prev, isProofOfStake);
    if (!prev1)
    {
      return targetBits; //first block;
    }

    const prev2 = this.getPrevCache(prev1,isProofOfStake) || await this.getPrevious(prev1, isProofOfStake);
    if (!prev2) {
      return targetBits; //second block
    }
    assert(prev2);
    ...
    let actualSpacing = prev.time - prev1.time;
    ...
    const compactNewTarget = consensus.toCompact(newTarget);
    return compactNewTarget;
  }
