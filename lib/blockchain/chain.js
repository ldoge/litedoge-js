...
class Chain extends AsyncEmitter {
  ...
  async connect(prev, block, flags) {
    ...
    try {
      const stakeModifier = this.computeNextStakeModifier(prev)
    } catch (e) {
      this.logger.error(e);
  ...
  async getLastStakeModifierEntry(entry) {
    assert(entry);
    while (entry && entry.prevBlock !== consensus.ZERO_HASH && !entry.generatedStakeModifier()) {
      const cache = this.getPrevCache(entry);
      if (cache)
        entry = cache;
      else
   ...
   *
   * @param prev
   * @returns {{stakeModifier:BigInt, generated: boolean}}
   */
  async computeNextStakeModifier(prev) {
    let modifier = 0n;
    let generatedStakeModifier = false;
    if (!prev) {
      return {stakeModifier: modifier, generated: generatedStakeModifier};
    }
    ...
    logger.debug("computeNextStakeModifier: prev modifier=%d time=%d", lastModifierEntry.stakeModifier, lastModifierEntry.time);
    if (lastModifierEntry.time / this.pos.modifierInterval >= prev.time / this.pos.modifierInterval) {
    ...
    let selectionInterval = this.getStakeModifierSelectionInterval();
    let selectionIntervalStart = (prev.time / this.pos.modifierInterval) * this.pos.modifierInterval - selectionInterval;

    const blockCandidates = new Map();
    let entry = prev;
    while (entry && entry.time >= selectionIntervalStart) {
      blockCandidates.set(entry.time, entry.hash());
      entry = this.getPrevCache(entry) || await this.getPrevious(entry);
    }
    const heightFirstCandidate = entry ? (entry.height + 1) : 0;
    const sortedBlockCandidates = new Map([...blockCandidates].sort())
    // Select 64 blocks from candidate blocks to generate stake modifier
    let stakeModifierNew = 0n;
    let selectionIntervalStop = selectionIntervalStart;

    const selectedBlocks = new Map();

    for (let round = 0; round < Math.min(64, sortedBlockCandidates.size); round++) {
      // add an interval section to the current selection round
      selectionIntervalStop += this.getStakeModifierSelectionIntervalSection(round);
      // select a block from the candidates of current round
      let candidate = this.selectBlockFromCandidates(sortedBlockCandidates, selectedBlocks, selectionIntervalStop, modifier);
      if (!candidate)
        throw new Error("ComputeNextStakeModifier: unable to select block at round " + round);
      // write the entropy bit of the selected block
      stakeModifierNew |= BigInt(entry.getStakeEntropyBit() << round);

      // add the selected block from candidates to selected list
      selectedBlocks.set(candidate.hash(), candidate);
      this.logger.context("stakemodifier")
        .info("ComputeNextStakeModifier: selected round %d stop=%s height=%d bit=%d", round, selectionIntervalStop, entry.height, entry.getStakeEntropyBit());
    }
    ...
    //     const CBlockIndex
    //   *)&
    //     item, mapSelectedBlocks
    //   )
    //     {
    //       // 'S' indicates selected proof-of-stake blocks
    //       // 'W' indicates selected proof-of-work blocks
    //       strSelectionMap.replace(item.second
    //     ->
    //       nHeight - nHeightFirstCandidate, 1, item.second
    //     ->
    //       IsProofOfStake() ? "S" : "W"
    //     )
    //       ;
  ...
  getStakeModifierSelectionIntervalSection(section) {
    ...
    return BigInt((this.pos.modifierInterval * 63 / (63 + ((63 - section) * (this.MODIFIER_INTERVAL_RATIO - 1)))));
  }

  /**
   *
   * @param {Map<Number, Hash>} sortedBlockCandidates
  ...
  async selectBlockFromCandidates(sortedBlockCandidates, selectedBlocks, selectionIntervalStop, prevStakeModifier) {
    ...
    let hashBest = 0n;
    let indexSelected;
    for (const [key, entryHash] of sortedBlockCandidates.entries()) {
      const entry = await this.getEntryByHash(entryHash)
      if (!entry)
        throw new Error("SelectBlockFromCandidates: failed to find block index for candidate block " + entryHash.toString('hex'));
      if (selected && entry.time > selectionIntervalStop)
        break;
      if (selectedBlocks.has(entryHash))
        continue;
      // compute the selection hash by hashing its proof-hash and the
      // previous proof-of-stake modifier
      // entry.chainwork << prevStakeModifier;
