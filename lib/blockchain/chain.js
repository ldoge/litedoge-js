...
class Chain extends AsyncEmitter {
  ...
  async connect(prev, block, flags) {
    ...
    try {
      const {stakeModifier, generated} = await this.computeNextStakeModifier(prev);
    ...
    try {
      entry.proofHash = await this.checkProofOfStake(prev, block.txs[1], entry.bits);
    } catch (e) {
      this.logger.error(e);
  ...
  async computeNextStakeModifier(prev) {
    ...
    for (let round = 0; round < Math.min(64, sortedBlockCandidates.length); round++) {
      ...
      modifier = modifier.uor(new BN(entropyBit).shl(new BN(round)));
  ...
  /**
   *
   * @param prev {ChainEntry}
   * @param {Block} block
   * @param bits {number}
   * @returns {Promise}
   */
  async checkProofOfStake(prev, block, bits) {
    const coinstake = block.txs[1];
    if (!coinstake.isCoinstake())
      throw new VerifyError(block, 'proofOfStake', 'called on non-coinstake', 0);

    // Kernel (input 0) must match the stake hash target per coin age (nBits)
    const input = coinstake.inputs[0];

    const coin = await this.getCoin(input.prevout.hash, input.prevout.index);
    if (!coin) {
      throw new VerifyError(block, 'proofOfStake', 'coin not found', 1);
    }

    //verify signature coin
    try {
      await this.workers.checkInput(coinstake, input.prevout.index, coin);
    } catch (e) {
      throw new VerifyError(block, 'proofOfStake', 'verify signature failed on coinstake  ' + coin.rhash(), 100);
    }

    const coinStakeInputEntry = await this.getEntryByHeight(coin.height);
    if (!coinStakeInputEntry) {
      throw new VerifyError(block, 'proofOfStake', 'coinstake input entry not found.', 1)
    }

    const coinStakeInputBlock = await this.getBlock(coinStakeInputEntry.hash);

    try {
      //Check stake kernel hash
      return this.checkStakeKernelHash(prev, bits, time, coinStakeInputBlock.txs[input.prevout.index], coin);
    } catch (e) {
      // may occur during initial download or if behind on block chain sync
      throw new VerifyError(block, 'proofOfStake', 'stake kernel check failed', 1)
    }
   ...
   * @param {Coin} coin
   */
  checkStakeKernelHash(prev, bits, time, txPrev, coin) {
    return this.network.isProtocolV1(prev.height) ?
      this.checkStakeKernelHashV1(prev, bits, time, txPrev, coin):
      this.checkStakeKernelHashV2(prev, bits, time, txPrev, coin)
      ;
  ...
  checkStakeKernelHashV1(prev, bits, time, txPrev, coin) {
    ...
    const targetProofOfStake = (bnCoinDayWeight * bnTargetPerCoinDay).getuint256();

    // Calculate hash
    CDataStream ss(SER_GETHASH, 0);
    uint64_tnStakeModifier = 0;
   ...
   * @param {Coin} coin
   */
  checkStakeKernelHashV2(prev, bits, time, txPrev, coin) {
    if (time < txPrev.time)
      throw new Error("checkStakeKernelHash(): nTime violation")
    ...
    const valueIn = txPrev.outputs[coin.index].value;
    const weightedTarget = target.mul(new BN(valueIn.toString()));

    const stakeModifier = prev.stakeModifier;
    const hashProofOfStake = this.getProofOfStakeHash(stakeModifier, prev.time, txPrev.time, coin, time);

    const targetProofOfStake = weightedTarget.toArrayLike(Buffer, 'le', 32);
    if (rcmp(hashProofOfStake, targetProofOfStake) > 0) {
      return null;
    }
   ...
   */
  getProofOfStakeHash(modifier, blockFromTime, coinTxTime, coin, time) {
    const bw = bio.write(56);
    bw.writeU64BI(modifier);
