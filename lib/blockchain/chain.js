...
class Chain extends AsyncEmitter {
  ...
  async connect(prev, block, flags) {
    ...
    if (entry.isProofOfStake()) {
      try {
        entry.proofHash = await this.checkProofOfStake(prev, block, entry.bits);
      } catch (e) {
        this.logger.error(e);
  ...
  async checkProofOfStake(prev, block, bits) {
    ...
    try {
      await this.workers.checkInput(coinstake, input.prevout.index, coin, common.flags.VERIFY_NONE);
    } catch (e) {
    ...
    try {
      ...
      return this.checkStakeKernelHash(prev, bits, coinStakeInputBlock, coinStakeInputBlock.txs[input.prevout.index], coin);
    } catch (e) {
   ...
   * @param {number} bits
   * @param {number} time
   * @param {Block} blockFrom
   * @param {TX} txPrev
   * @param {Coin} coin
   */
  checkStakeKernelHash(prev, bits, blockFrom, time, txPrev, coin) {
    return this.network.isProtocolV1(prev.height) ?
      this.checkStakeKernelHashV1(prev, bits, time, txPrev, coin) :
      this.checkStakeKernelHashV2(prev, bits, blockFrom.time, txPrev, coin);
  }
  ...
  async getKernelStakeModifier(hashBlockFrom) {
    const entryFrom = await this.getEntryByHash(hashBlockFrom);
    if (!entryFrom)
    ...
    while (time < entry.time + stakeModifierSelectionInterval) {
      if (entry.height === this.tip.height) {
  ...
  checkStakeKernelHashV1(prev, bits, time, txPrev, coin) {
    ...
    if (new BN(hashProofOfStake).gt(weightedTarget))
      return null;
  ...
  checkStakeKernelHashV2(prev, bits, time, txPrev, coin) {
    ...
    const hashProofOfStake = this.getProofOfStakeHashV2(stakeModifier, prev.time, txPrev.time, coin, time);
