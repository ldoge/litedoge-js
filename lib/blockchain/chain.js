...
class Chain extends AsyncEmitter {
  ...
  async getTarget(time, prev, isProofOfStake) {
    //TODO:: this function fails after block 545
    const pow = this.network.pow;
    ...
    const prev1 = this.getPrevCache(prev) || await this.getPrevious(prev);
    if (!prev1)
    {
      return pow.bits; //first block;
    }

    const prev2 = this.getPrevCache(prev1) || await this.getPrevious(prev1);
    if (!prev2) {
      return pow.bits; //second block
    }
    assert(prev2);
