...
class Chain extends AsyncEmitter {
  ...
  async connect(prev, block, flags) {
    ...
    if (entry.isProofOfStake()) {
      try {
        let {proofHash} = await this.checkProofOfStake(prev, block.txs[1], entry.bits);
   ...
   * @param {primitives.TX} coinstakeTx
   * @param bits {number}
   * @returns {Promise}
   */
  async checkProofOfStake(prev, coinstakeTx, bits) {
    ...
    const coinStakeEntry = await this.getEntryByHeight(coin.height);
    if (!coinStakeEntry) {
      throw new VerifyError(coinStakeEntry, 'proofOfStake', 'coinstake input entry not found.', 1)
    }
    const coinStakeInputBlock = await this.getBlock(coinStakeEntry.hash);
    ...
    try {
      ...
      return this.checkStakeKernelHash(prev, bits, coinStakeInputBlock, inputTransaction, input.prevout,coinstakeTx.time,  coin);
    } catch (e) {
