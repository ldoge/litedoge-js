...
class Chain extends AsyncEmitter {
  ...
  async verify(block, prev, flags) {
    ...
    if (!block.isProofOfStake()) {
      ...
      // let [hashProof, targetProofOfStake] = this.checkProofOfStake(prev, block, bits);
    }
  ...
  async connect(prev, block, flags) {
    ...
    try {
      const stakeModifier = await this.computeNextStakeModifier(prev)
      entry.setStakeModifier(stakeModifier.stakeModifier, stakeModifier.generated)
    } catch (e) {
      this.logger.error(e);
  ...
  async computeNextStakeModifier(prev) {
    ...
    for (let round = 0; round < Math.min(64, sortedBlockCandidates.length); round++) {
      ...
      //TODO:: THERE IS SOMETHING WRONG WITH THIS ALGORITHM
   ...
   * @param {Block} block
   * @param bits {number}
   * @returns {Promise<[BN, BN]>}  [ProofOfStakeHash, StakeModifier]
   */
  async checkProofOfStake(prev, block, bits) {
    ...
    this.checkStakeKernelHash(prev, bits, time, coinStakeInputBlock.txs[input.prevout.index], coin);
  }
  ...
  checkStakeKernelHash(prev, bits, time, txPrev, coin) {
    ...
    const weightedTarget = target * new BN(valueIn.toString());


    const targetProofOfStake = weightedTarget.toArrayLike(Buffer, 'le', 32)
    const stakeModifier = prev.stakeModifier;
    const stakeModifierHeight = prev.height;
    const stakeModifierTime = prev.time;

    const proofOfStakeHash = this.getProofOfStakehash(stakeModifier, prev.time, txPrev.time, coin, time);
    return true;
  }
