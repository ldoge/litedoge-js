...
class ChainDB {
  ...
  async getPrevious(entry, proofOfStake) {
    let prev = await this.getEntryByHash(entry.prevBlock);
    while (prev && prev.prevBlock && prev.isProofOfStake() !== proofOfStake) {
      prev = await this.getEntryByHash(prev.prevBlock);
    }
    return prev;
  }
   ...
   * @param {ChainEntry} entry
   * @param {Boolean} proofOfStake the entry is proof of stake
   * @returns {ChainEntry|null}
   */

  getPrevCache(entry, proofOfStake = false) {
    let prev = this.cacheHash.get(entry.prevBlock);
    while (prev && prev.prevBlock && prev.isProofOfStake() !== proofOfStake) {
      prev = this.cacheHash.get(prev.prevBlock);
    }
    return prev || null;
  }
  ...
  async _save(entry, block, view) {
    ...
    // Hash->height index.
    this.put(layout.h.encode(hash), fromU16(entry.height));
...
class ChainState {
  ...
  toRaw() {
    ...
    //current implmentation of this causes issues might need to switch to BigInt
