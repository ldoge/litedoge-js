...
class ThreadStaker extends EventEmitter {
  ...
  constructor(stakingKernel) {
    ...
    this.reserveBalance = this.kernel.options.reserveBalance;
    this.running = false;
    this.stopping = false;
    this.job = null;
    this.stopJob = null;
  ...
  async _start() {
    assert(!this.running, 'ThreadStakeMiner is already running.');
    this.wallet = this.kernel.wallet;
    ...
    for (; ;) {
      ...
      let block;
      try {
        block = await this.signAsync(this.job);
      } catch (e) {
        if (this.stopping)
          break;
        this.emit('error', e);
  ...
  /**
   * Sign asynchronously until the block is found.
   * @method
  ...
  async signAsync(job) {
    ...
    for (; ;) {
      ...
      const isProtocolV2 = this.network.isProtocolV2(tip.height + 1);
      let coinStakeTime = util.now();
      // new transaction
      if (isProtocolV2) {
        coinStakeTime &= ~this.kernel.STAKE_TIMESTAMP_MASK;
      }

      if (true || coinStakeTime > job.lastSearchTime) {
        let searchInterval = isProtocolV2 ? 1 : coinStakeTime - job.lastSearchTime;
        //might need to be async
        const coinStake = await this.createCoinStake(job.attempt, coinStakeTime, searchInterval);
  ...
  /**
   *
   * @param {BlockTemplate} job
   * @param {number} time
   * @param {number} searchInterval
   * @returns {module:primitives.TX|module:primitives.TX}
   */
  async createCoinStake(job, time, searchInterval) {
    let prev = this.chain.tip;
    const targetPerCoinDay = consensus.fromCompact(job.bits);
    const coinStakeTx = new TX({time: time});

    //Mark coin stake transaction
    coinStakeTx.outputs.push(new Output());


    //Choose coins to use
    const balance = await this.wallet.getBalance(0);
    if (balance.confirmed <= this.reserveBalance) {
      throw new Error("No balance available for coinstake.");
    }

    const targetBalance = balance.confirmed - this.reserveBalance;
    const unReserved = await this.getUnreservedStakingCoins(targetBalance);
    if (!unReserved || unReserved.length === 0) {
      throw new Error("No balance available for coinstake.");
    }
    ...
    for (const credit of unReserved) {
      //Get the block
      const chainEntry = await this.chain.getEntry(credit.coin.height);
      const prevTx = await this.wallet.txdb.getTX(credit.coin.hash);

      if (chainEntry.time + this.network.pos.stakeMinAge > coinStakeTx.time - maxStakeSearchInterval)
        continue; // only count coins meeting min age requirement

      let kernelFound = false;
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prev === this.chain.tip; n++) {
        if (this.stopping) {
          throw new Error("Stopping");
        }
        ...
        this.kernel.checkStakeKernelHash(prev, job.bits, chainEntry, prevTx.tx, credit.coin)
      }
  ...
  async getUnreservedStakingCoins(targetBalance) {
    const stakableCredits = await this.wallet.getStakableCredits(0)
