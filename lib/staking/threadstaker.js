...
class ThreadStaker extends EventEmitter {
  ...
  async signAsync(job) {
    ...
    for (; ;) {
      ...
      if (coinStakeTime > job.lastSearchTime) {
  ...
  async createCoinStake(job, time, searchInterval) {
    ...
    const targetPerCoinDay = consensus.toCompact(job.bits);
    ...
    const targetBalance = balance - this.reserveBalance;
    ...
    for (const credit of unReserved) {
      ...
      for (let n = 0; n < Math.min(searchInterval, maxStakeSearchInterval) && !kernelFound && prev === this.chain.tip; n++) {
        ...
        this.kernel.checkStakeKernelHash(prev, job.bits, chainEntry, credit)
      }
