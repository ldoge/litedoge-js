  ...
  async attemptStakeAsync(job) {
    ...
    if (searchTime > job.lastSearchTime) {
      ...
      if (coinStake != null) {
        ...
        // if (coinStake.time >= Math.max(pindexBest->GetPastTimeLimit() + 1, PastDrift(pindexBest->GetBlockTime(), pindexBest->nHeight + 1))) {
        //   // make sure coinstake would meet timestamp protocol
        //   // as it would be the same as the block timestamp
        //   let time = block.txs[0].time = coinStake.time;
        //   time = Math.max(pindexBest->GetPastTimeLimit() + 1, GetMaxTransactionTime());
        //   time = Math.max(block.time, PastDrift(pindexBest->GetBlockTime(), pindexBest->nHeight + 1));
        //
